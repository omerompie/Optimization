...
def main():
    ...
    margin = 2.0  # graden extra ruimte (handig voor interpolatie / rounding)
    bottomlat = min(lats) - margin
    toplat = max(lats) + margin
    leftlon = min(lons) - margin
    rightlon = max(lons) + margin

    print("Bounding box (NOMADS subregion):")
    print(f"  leftlon   = {leftlon:.3f}")
    print(f"  rightlon  = {rightlon:.3f}")
    print(f"  toplat    = {toplat:.3f}")
    print(f"  bottomlat = {bottomlat:.3f}")

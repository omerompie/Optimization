...
def plot_trajectories(best_solution, great_circle_trajectory, node_coords, out_html="compare_routes.html"):
    # ids -> (lat,lon)
    route_best = [node_coords[nid] for nid in best_solution]
    route_gc = [node_coords[nid] for nid in great_circle_trajectory]

    # map
    m = folium.Map(location=route_best[0], zoom_start=4, tiles="CartoDB dark_matter")
    ...
    # Best solution line (red)
    folium.PolyLine(
        locations=route_best,
        color="#ff3b30",
        weight=4,
        opacity=0.95
    ).add_to(m)

    # Start/End markers (gebruik start/eind van best_solution)
    folium.Marker(
        location=route_best[0],
        popup=f"Start (node {best_solution[0]})",
        icon=folium.Icon(color="green", icon="play")
    ).add_to(m)

    folium.Marker(
        location=route_best[-1],
    ...
    # fit bounds zodat alles goed in beeld is (combineer beide routes)
    all_points = route_best + route_gc
...
def main():
    ...
    plot_trajectories(best_solution, great_circle_trajectory, node_coords)
